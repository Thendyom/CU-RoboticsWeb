...
async function loadZoteroPubs() {
  const GROUP_ID = window.ZOTERO_GROUP_ID || 'REPLACE_ME'; // e.g., 1234567
  const target = document.getElementById('pubs');
  if (!target) return;

  if (!GROUP_ID || GROUP_ID === 'REPLACE_ME') {
  ...
  try {
    ...
    const cards = items
      ...
      .map(it => {
        ...
        const venue = d.publicationTitle || d.proceedingsTitle || d.conferenceName || d.journalAbbreviation || '';
        const date = (d.date || '').slice(0,4);
        const authors = (d.creators || []).map(c => (c.lastName||'') + (c.firstName? ', ' + c.firstName[0] + '.':'')).join('; ');
        ...
        return `<div class="pub-card">
          ...
          <p class="pub-meta">${authors}${venue? ' — ' + venue : ''}${date? ' ('+date+')' : ''}</p>
          <p class="pub-links">
